...
class ClientController extends Controller
{
    ...
    {
        $data = LoanApplication::where('borrower_id', $request->session()->get('uid'))->get();
        $borrower = Borrower::find($request->session()->get('uid'));
        return view('client.applications', compact('data', 'borrower'));
    }
    public function guideApplication(Request $request)
    {
        $data = LoanApplication::where('borrower_id', $request->session()->get('uid'))->get();
        $borrower = Borrower::find($request->session()->get('uid'));
        return view('client.guide', compact('data', 'borrower'));
    }
